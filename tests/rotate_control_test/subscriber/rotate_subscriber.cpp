...
CPlcRotateStateSubscriber *gpRotateSubscriber = nullptr;
...
{
    LOG_INFO("Status [%d]", gpRotateSubscriber->GetStatus());
    LOG_INFO("Actual Rate [%f]", gpRotateSubscriber->GetActualRate());
    LOG_INFO("Min Rate [%f]", gpRotateSubscriber->GetMinRate());
    LOG_INFO("Max Rate [%f]", gpRotateSubscriber->GetMaxRate());
    LOG_INFO("Target Rate [%f]", gpRotateSubscriber->GetTargetRate());
}
...
int32_t main(int32_t argc, char **argv)
{
    ...
    gpRotateSubscriber = new CPlcRotateStateSubscriber();

    if (gpRotateSubscriber->Create(gDomain) == true)
    {
        gpRotateSubscriber->OnDataAvailable([&](const DDS::SampleInfo &sampleInfo)
                                             {
                                                 DataAvailable(sampleInfo);
                                             });

        gpRotateSubscriber->OnSubscriptionMatched([&](const DDS::SubscriptionMatchedStatus &status)
                                                 {
                                                     SubscriptionMatched(status);
                                                 });

        gpRotateSubscriber->OnDataDisposed([&](const DDS::SampleInfo &sampleInfo)
                                          {
                                              DataDisposed(sampleInfo);
                                          });

        gpRotateSubscriber->OnLivelinessChanged([&](const DDS::LivelinessChangedStatus &status)
                                               {
    ...
    LOG_INFO("PLC RotateState subscriber started");
